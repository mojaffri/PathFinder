...
- **Every AI path has a real, fully-functional fallback**, not a degraded error state. `lib/resume/ai-extractor.ts`, `lib/roadmap/ai-generator.ts`, and `lib/skillforge/ai-evaluator.ts` all return `null` on any failure (missing key, network error, schema-invalid response) — never throw — and every caller has a deterministic/heuristic path ready to take over (`lib/resume/heuristic-extractor.ts`, `lib/roadmap/fallback.ts`; SkillForge grading has no fallback since a human answer can't be graded without the AI, but the route still always returns HTTP 200 with `{ evaluation: null }` and persists the raw answer). Any new AI integration must follow this contract.
...
- **Never expose the Anthropic API key client-side.** `lib/ai/anthropic-client.ts` is the only place `ANTHROPIC_API_KEY` is read, and it must only ever be imported from a route handler (`app/api/**/route.ts`), never from a `"use client"` component. The same rule applies to `SUPABASE_SERVICE_ROLE_KEY` (`lib/supabase/admin.ts`, marked with the `server-only` package) and `DATABASE_URL` (`lib/db/client.ts`).
...
- Do not add a test suite that mocks the AI provider's *behavior* in a way that could mask a real prompt/schema mismatch — mock at the `getAnthropicClient()`/`client.messages.create` boundary, not by faking `generateRoadmapWithAI`'s return value.
