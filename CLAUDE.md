...
- **Every AI path has a real, fully-functional fallback**, not a degraded error state. Text-based resume extraction and roadmap generation return `null` on provider failure and fall back to their heuristic/deterministic implementations. Scanned/image-only PDFs require configured document-capable AI because they contain no text for a deterministic parser; return a specific actionable error when it is unavailable. SkillForge's current catalog is fixed-choice and deterministic; future genuinely open responses may use AI. New AI integrations must preserve useful non-AI behavior.
...
- **Normalize every resume extraction before validation/persistence.** Both AI and heuristic results pass through `normalizeResumeExtraction()` so layout artifacts, orphan project/experience/award descriptions, reversed role/company fields, URLs, and duplicate bullets obey one record contract. Validated PDFs may also be passed as server-only document blocks so AI can use visual layout and read scanned pages; never log those bytes.
- **Never expose AI credentials client-side.** Provider access is server-only through `lib/ai/anthropic-provider.ts`/`structured-output.ts`; the adapter prefers a direct Anthropic key and otherwise uses Vercel AI Gateway through `AI_GATEWAY_API_KEY` or the deployment's short-lived `VERCEL_OIDC_TOKEN`. Legacy job/GitHub extractors still use the server-only `lib/ai/anthropic-client.ts` wrapper. None may be imported from a `"use client"` component. The same rule applies to `SUPABASE_SERVICE_ROLE_KEY` (`lib/supabase/admin.ts`) and `DATABASE_URL` (`lib/db/client.ts`).
