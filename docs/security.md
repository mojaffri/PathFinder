...
- The stateless `/api/skillforge/evaluate` calculation is the only product POST allowed through the demo guard so a recruiter can try an assessment. Saving the attempt remains blocked; the result is visibly labeled temporary and the seeded mastery state never changes.
