...
function SkillDetailBody({
  ...
  // root-cause backtracking â€” in that case the banner must link to that
    ...
    <div className="mx-auto max-w-3xl px-6 py-16">
          ...
          <div>
            ...
            {nbaBacktrack && (
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                â€” a prerequisite skill, not this one.
              </p>
            )}
          </div>
          {nbaBacktrack ? (
      ...
      {!readiness.ready && !readinessDismissed && (
        <div className="mt-6 rounded-md border border-warning bg-warning-bg p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-warning">Heads up â€” not a block</p>
 ...
 ãŸm¢G§²ÚîÆ­yÓt title="Examples" items={skillModule.learnOverview.examples} />
      ...
      <Card className={`mt-6 ${activeStage !== "prove" ? "hidden" : ""}`} id="assessment-section">
        <CardHeader>
          <CardTitle className="text-base">Prove it â€” mastery assessment</CardTitle>
      ...
      <Card className={`mt-6 ${activeStage !== "prove" ? "hidden" : ""}`}>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Real, credible artifacts backing this skill â€” a repo, a document, a link.</p>
        </CardHeader>
        <CardContent>
    ...
    <div className="flex items-start gap-3 rounded-md bg-surface p-3">
      ...
      <div className="flex-1">
        ...
        {resource.url ? (
          ...
          <p className="mt-1 text-xs italic text-muted-foreground">No verified link yet â€” search for this by title.</p>
        )}
      </div>
    </div>
  );
  ...
  function commit() {
    ...
    onProgressChange(
      addEvidence(userId, skillModule, {
        ...
        // A student-added artifact starts as "moderate" â€” genuinely stronger
